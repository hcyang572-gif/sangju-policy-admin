...
// v47 사유 (2026-08-25, 🔵손길) — 푸터에 「바탕화면 아이콘 설치」 단추 추가.
//   index.html·style.css·a2hs.js 를 «셋 다» 고쳤다. 셋 모두 ESSENTIAL 프리캐시 대상이라
//   캐시 이름을 올리지 않으면 «이미 설치된 공무원앱»은 v46 캐시의 옛 a2hs.js·style.css 를
//   계속 써서 단추가 아예 나타나지 않는다(「고쳤는데 안 보인다」 — 이 저장소가 여러 번 겪은 사고).
//   ⚠ 🟣나루에게 — 배포 회차의 버전을 올린 «직후» `py -3 자원버전_동기화.py` 를 반드시 한 번
//     돌려 주세요. 지금 index.html 의 ?v= 와 아래 ASSET_V 는 «이미 배포된» 0.7.4 그대로입니다.
//     CACHE 이름만 올려도 프리캐시는 cache:"reload" + swcb 쿼리로 HTTP 캐시를 우회해 새 파일을
...
const CACHE = "sangju-admin-v57";   // v57: version.js 의 changelog v0.7.6 date 를 8/25→8/26 로
                                     ...
                                     //   공무원앱이 옛 여백을 계속 쓴다(「고쳤는데 그대로다」).
                                     // v50: 둘러보기(게스트)에게서 «지우는 길»을 걷어냄 — 지원사업·접수건·
                                     //   서식 삭제 단추를 게스트에게 아예 내보내지 않고, 쓰기 차단에
                                     //   sb.rpc 를 더했다(app.js, 2026-08-25 양호창님 지시).
                                     //   ⚠ 서버(권한정리_260824.sql)는 이미 막고 있었다 — 이 판은 «화면» 쪽이라
                                     //   옛 app.js 가 캐시에 남으면 단추가 그대로 보인다. 이름을 올려야 반영된다.
                                     // v49: 앱 아이콘 ②안 교체 — 로고 334px 등배, icon-admin-*-v4.png(2026-08-25, 🔵손길).
                                     // v48: 초점 관리 보강 — 「바탕화면 아이콘 설치」 단추를 감출 때
                                     //   초점을 이웃 푸터 단추로 넘기고(a2hs.js), 댓글 감추기·삭제 뒤
                                     //   목록을 다시 그릴 때 초점을 되돌린다(app.js pCmtRefocus).
                                     //   ⚠ 둘 다 «이미 설치된» 앱에서는 옛 a2hs.js·app.js 가 v47 캐시에
                                     //   남아 그대로 초점을 잃는다 — 캐시 이름을 반드시 올려야 반영된다.
                                     // v47: 푸터 「바탕화면 아이콘 설치」 단추(index.html·style.css·a2hs.js).
                                     // v46: 0.7.4 — 둘러보기 게스트 분기·읍면동 카드 문구 축약·실시간 연결 표시 등(자원버전 동기화 반영).
                                     // v43: 0.7.1 — 정책제안 ⓘ 안내 문단 제거.
...
const OPTIONAL = [
  "assets/icon-admin-192-v4.png",
  "assets/icon-admin-512-v4.png",
